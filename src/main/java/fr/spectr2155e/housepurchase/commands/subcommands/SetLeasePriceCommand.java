...
public class SetLeasePriceCommand extends SubCommand {
    ...
    public void perform(Player player, String[] args) {
        if(!(args.length == 3)){
            player.sendMessage("§8§l(§4§lERREUR§8§l) §cVeuillez utiliser la commande /housepurchase setBuyLease <prix>/j <id>.");
            return;
        }
        ...
        player.sendMessage("§8§l(§6§lHousePurchase§8§l) §fLe prix de la maison à la location sous l'id §e"+id+" §fest maintenant de §a"+price);
    }
}
